...
- **2026-09-18** — Home real (`GET /api/v1/home`) alimentando "Dicas para você" (antes batia num `/dicas` que nunca existiu, sempre 404); `conteudos`/`revisao` de `GET /diagnosticos/{id}` (já existiam no back, nunca eram consumidos) agora aparecem no detalhe do diagnóstico e na tela de resultado do wizard; card de classificação+foto do detalhe redesenhado (empilhado no mobile, lado a lado no web) mostrando a foto real capturada; anamnese voltou a ser colapsável no web (bug de CSS deixava sempre aberta); anamnese do wizard bloqueia avançar em pergunta obrigatória sem resposta; máscara de telefone no registro; ícone por nível de classificação (Smile/Meh/Frown). PR #27, ainda sem revisão.
- **2026-09-18** — Redesenho da tela de captura da língua (`components/captura/`): moldura 4:3 com guia em formato de língua, contagem regressiva de 3 s e flash antes da foto, prévia na mesma moldura e botões próprios para web e celular. Se a câmera ao vivo falha, a tela avisa e o próximo toque abre a câmera nativa. Com foto da galeria, a câmera desliga na revisão.
- **2026-09-18** — Integração do fluxo de diagnóstico com a API real: captura da língua com câmera ao vivo (traseira/frontal) ou galeria, envio em multipart para `POST /diagnosticos`, polling de `GET /diagnosticos/{id}` até o resultado sair e lista "Meus diagnósticos" paginada via `GET /diagnosticos` com filtro de período no servidor. O nível exibido é a `classificacao.ordem` do back (1 Hálito Normal, 2 Halitose Íntima, 3 Mau Hálito Social). A câmera ao vivo exige HTTPS ou `localhost`; fora disso, o botão cai na câmera nativa do celular.
