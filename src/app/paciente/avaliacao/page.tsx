...
/**
 * Fluxo de autoavaliação do paciente — 9 passos internos, uma rota só
 * (como em Design/'s DiagnosisFlow), com o estado do wizard em memória.
 * Sem persistência em sessionStorage ainda (a fase 2 original do plano
 * previa rota por passo + sessionStorage — simplificado aqui pra cobrir
 * a tela toda primeiro; dá pra evoluir depois sem mudar a UI).
 *
 * TODO: `PACIENTE_ID_PLACEHOLDER` deve virar o id do paciente logado
 * assim que existir um hook "usuário atual" (ver sessão em
 * app/paciente/layout.tsx -> RoleLayout, que já sabe o id mas ainda não
 * expõe pro cliente).
 */
...
export default function AvaliacaoPage() {
  ...
  const back = () => (step > 0 ? setStep((s) => s - 1) : router.push("/paciente"));
  ...
  async function confirmarAnamneseECaptura() {
    ...
    const diagnostico = await criarDiagnostico.mutateAsync({
      pacienteId: PACIENTE_ID_PLACEHOLDER,
      imagemUrl: "",
      ...
      <div className="flex-1 overflow-y-auto p-4">
        ...
        {step === 6 && resultado && (
          <div className="flex flex-col gap-4">
            ...
            <Button variant="secondary" onClick={() => router.push("/paciente")}>
              Voltar ao início
            </Button>
          </div>
        )}
